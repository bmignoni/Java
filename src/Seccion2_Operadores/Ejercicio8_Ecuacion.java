/* Ejercicio 8: Construir un programa que calcule y muestre por pantalla las
raices de la ecuación de segundo grado de coeficientes reales. 
ax^2 + bx + c = 0  
 */
package Seccion2_Operadores;

import java.util.Scanner;

public class Ejercicio8_Ecuacion {
    
    public static void main (String[] args){
    
    Scanner entrada = new Scanner(System.in);
        
        /* el float ocupa menos espacio de memoria que el double 
        pero la clase Math solo permite double por lo que se usa asi*/
        
        double a, b, c, resultado1, resultado2;
                
        System.out.print("Digite el valor de a: ");
        a = entrada.nextDouble();
        System.out.print("Digite el valor de b: ");
        b = entrada.nextDouble();
        System.out.print("Digite el valor de c: ");
        c = entrada.nextDouble();
        
        resultado1 = (-b + Math.sqrt(Math.pow(b, 2) - (4*a*c))) / (2*a);
        resultado2 = (-b - Math.sqrt(Math.pow(b, 2) - (4*a*c))) / (2*a);
        
        System.out.println("\nLa raiz 1 es : " + resultado1);
        System.out.println("\nLa raiz 2 es : " + resultado2);
   
    }
}
